...
try {
    ...
    console.log('TEST 12: updateStatus - Rejects non-existent record');
    {
        try {
            ...
            assert(e.message.includes('Record not found'), `Wrong error: ${e.message}`);
    ...
    console.log('TEST 16: getAllItems - Returns all expected fields');
    {
        ...
        assert(item.total_amount !== undefined, 'Missing total_amount');
    ...
    console.log('TEST 17: getAllItems - Handles special cases (silver_cert total=0)');
    {
        ...
        assert.strictEqual(item.total_amount, 0, 'Silver certificate total should be 0 (by design)');
