...
const NewSilverTestModal = ({ show, onHide, onSuccess }) => {
    ...
    useEffect(() => {
        if (show) {
            fetchCustomers();
            if (!selectedCustomer) {
                setItems([{ name: '', item: 'Silver Sample', weight: '', sample_weight: '', returned: false }]);
    ...
    const handleCustomerSelect = (customer) => {
        ...
        // Auto-fill first item name if empty or default
        setItems(prev => {
            const newItems = [...prev];
    ...
    const addItem = () => {
        ...
        setItems([...items, { name, item: 'Silver Sample', weight: '', sample_weight: '', returned: false }]);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success('Silver Test Created Successfully');
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered className="new-test-modal">
            <Modal.Header closeButton className="border-0 pb-0">
                <Modal.Title className="fw-bold">New Silver Test</Modal.Title>
            </Modal.Header>
            <Modal.Body>
